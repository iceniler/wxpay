...
ZEPHIR_INIT_CLASS(WXPay_Api) {
	...
	zend_declare_property_string(wxpay_api_ce, SL("api_url"), "", ZEND_ACC_PRIVATE TSRMLS_CC);

	zend_declare_property_bool(wxpay_api_ce, SL("is_secure_pay"), 0, ZEND_ACC_PRIVATE TSRMLS_CC);

	zend_declare_property_null(wxpay_api_ce, SL("_required_properties"), ZEND_ACC_PRIVATE TSRMLS_CC);

	zend_declare_property_null(wxpay_api_ce, SL("_optional_properties"), ZEND_ACC_PRIVATE TSRMLS_CC);

	zend_declare_property_null(wxpay_api_ce, SL("_either_groups"), ZEND_ACC_PRIVATE TSRMLS_CC);

	zend_declare_property_null(wxpay_api_ce, SL("_config"), ZEND_ACC_PRIVATE TSRMLS_CC);
...
PHP_METHOD(WXPay_Api, signWith) {
	...
	if (!(zephir_instance_of_ev(config, wxpay_config_ce TSRMLS_CC))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(zend_exception_get_default(TSRMLS_C), "Invalid \\WXPay\\Config!", "wxpay/api.zep", 37);
		return;
	}
	_0 = !((0 == 0));
	if (!(_0)) {
	...
	zephir_is_iterable(_5, &_7, &_6, 0, 0, "wxpay/api.zep", 75);
	for (
	  ...
	  ; zephir_hash_move_forward_ex(_7, &_6)
		...
		if (zephir_fast_in_array(field, _9$$5 TSRMLS_CC)) {
			...
			zephir_is_iterable(_10$$6, &_12$$6, &_11$$6, 0, 0, "wxpay/api.zep", 55);
			for (
			...
			if (!(group_through)) {
				...
				zephir_array_append(&messages, _15$$9, PH_SEPARATE, "wxpay/api.zep", 57);
			} else {
				continue;
			}
		}
		...
		if (_18$$5) {
			zephir_array_append(&messages, field, PH_SEPARATE, "wxpay/api.zep", 66);
		}
		...
		if (_20$$5) {
			zephir_array_append(&messages, field, PH_SEPARATE, "wxpay/api.zep", 71);
		}
	}
	if (zephir_fast_count_int(messages TSRMLS_CC) > 0) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(_22$$13, "wxpay/api.zep", 76 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_OBS_VAR(_25);
...
PHP_METHOD(WXPay_Api, dump) {
	...
	zephir_is_iterable(mergedArray, &_4, &_3, 0, 0, "wxpay/api.zep", 97);
	for (
...
PHP_METHOD(WXPay_Api, fire) {
	...
	HashTable *_4;
	HashPosition _3;
	zval *responseArray = NULL, *property = NULL, *xml = NULL, *_0, *_1, *_2 = NULL, **_5, *_9, *_6$$3 = NULL, *_7$$4 = NULL, *_11$$5, *_12$$5, *_13$$5, *_14$$5, *_15$$5, *_16$$6;
	zval *parameters, *mergedArray = NULL;
	...
	zephir_check_call_status();
	ZEPHIR_CPY_WRT(mergedArray, _2);
	zephir_is_iterable(mergedArray, &_4, &_3, 0, 0, "wxpay/api.zep", 115);
	for (
	  ; zephir_hash_get_current_data_ex(_4, (void**) &_5, &_3) == SUCCESS
	  ; zephir_hash_move_forward_ex(_4, &_3)
	) {
	...
	ZEPHIR_CALL_CE_STATIC(&xml, wxpay_library_util_ce, "convertarraytoxml", &_8, 10, parameters);
	zephir_check_call_status();
	_9 = zephir_fetch_nproperty_this(this_ptr, SL("is_secure_pay"), PH_NOISY_CC);
	if (zephir_is_true(_9)) {
		_11$$5 = zephir_fetch_nproperty_this(this_ptr, SL("api_url"), PH_NOISY_CC);
		_12$$5 = zephir_fetch_nproperty_this(this_ptr, SL("_config"), PH_NOISY_CC);
		ZEPHIR_OBS_VAR(_13$$5);
		zephir_read_property(&_13$$5, _12$$5, SL("client_cert"), PH_NOISY_CC);
		_14$$5 = zephir_fetch_nproperty_this(this_ptr, SL("_config"), PH_NOISY_CC);
		ZEPHIR_OBS_VAR(_15$$5);
		zephir_read_property(&_15$$5, _14$$5, SL("client_key"), PH_NOISY_CC);
		ZEPHIR_CALL_CE_STATIC(&responseArray, wxpay_library_util_ce, "callwechatapi", &_10, 11, _11$$5, xml, _13$$5, _15$$5);
		zephir_check_call_status();
	} else {
		_16$$6 = zephir_fetch_nproperty_this(this_ptr, SL("api_url"), PH_NOISY_CC);
		ZEPHIR_CALL_CE_STATIC(&responseArray, wxpay_library_util_ce, "callwechatapi", &_10, 11, _16$$6, xml);
		zephir_check_call_status();
	}
